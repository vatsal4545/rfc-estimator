...
const FIXTURE = join(__dirname, "..", "__fixtures__", "intake-sample-3.2.0.xlsx");
...
describe("importing a completed intake workbook", async () => {
  ...
  it("recognises the workbook and records its version", () => {
    ...
    expect(report.contentHash).toBe("b0140566d4234d54");
  ...
  it("Project tab → setup and intake", () => {
    ...
    expect(it.notes).toMatch(/Imported from EVSE Project Intake 3\.2\.0 Rev A completed by Test CPM on 2026-09-02/);
  });
