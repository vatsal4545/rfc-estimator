...
describe("filling the CEO's intake from a project", async () => {
  ...
  it("Version and Project tabs", () => {
    ...
    expect(wb.get("Version", "B4")).toBe("3.8.1"); // untouched
  ...
  it("refuses a template the cell map was not written for", async () => {
    ...
    await expect(fillIntakeWorkbook(doctored.bytes, project, result, proposal)).rejects.toThrow(/does not match the app's cell map for 3\.8\.1/);
  ...
  it("plan helpers and vocabularies", () => {
    ...
    expect(intakeFileName(project)).toBe("best-western-hawthorne-evse-intake-3.8.1-rev-b.xlsx");
