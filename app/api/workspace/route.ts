...
function libraryPath(key: string): string {
  const h = createHash("sha256").update(`rfc-estimator:${key}`).digest("hex");
  return `ws/${h}/library.json`;
}

function keyFrom(req: Request): string | null {
  const k = req.headers.get("x-workspace-key")?.trim();
  return k && k.length >= 6 ? k : null;
}

export async function GET(req: Request) {
  const key = keyFrom(req);
  if (!key) return Response.json({ error: "workspace key required (min 6 characters)" }, { status: 401 });
  try {
    const res = await get(libraryPath(key), { access: "private", useCache: false });
...
export async function PUT(req: Request) {
  ...
  try {
    await put(libraryPath(key), body, {
      access: "private",
      allowOverwrite: true,
